...
import { confirmEmail, checkEmailToken, resetPassword } from "../controllers/mailService";
...
emailRoute.get("/v=:token", async (req: Request, res: Response) => {
    ...
    else // Invalid token or error occurred
        res.sendStatus(404);
...
emailRoute.get("/r=:token/p=:newpw", (req: Request, res: Response) => {
    ...
    if (decodedToken != null)
        resetPassword(decodedToken, req.params.newpw);
